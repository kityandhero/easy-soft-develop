...
import { globalScript, packageScript } from '../tools/package.script.js';
import {
  getGlobalDevelopPackages,
  getMainDevelopPackages,
  getProjectDevelopPackages,
} from '../tools/package.dependence.js';
import { createDevelopInitialEnvironmentConfigFile } from '../config/develop.initial.environment.js';
import { createDevelopSubPathVersionNcuConfigFile } from '../config/develop.subPath.version.ncu.js';
import { createDevelopUpdateProjectFromRepositoryConfigFile } from '../config/develop.update.project.from.repository.js';
...
function createRepositoryPackageJsonFile(repositoryName) {
  ...
  packages = [...packages, ...getMainDevelopPackages()];
...
function createPackageJsonFile(name) {
  ...
  packages = [...packages, ...getProjectDevelopPackages()];
...
function createHusky() {
  ...
  const commitMsg = String.raw`echo ---------------------
...
`;
  ...
  const precommit = String.raw`echo ---------------------
...
function initialEnvironment() {
  ...
  exec(
    'npx npm-check-updates --configFilePath ./.ncurc.mjs --packageFile ./**/package.json -u',
  );
...
export function createRepositoryProject(name) {
  ...
  createNcuConfigFile(`step *: create .ncurc.mjs success`);
